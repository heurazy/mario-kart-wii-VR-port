...
This update fixes recurring controller-search hitches, incorrect startup placement of spatial VR menus, and black screens when opening VR settings or the controller tutorial during a race.
...
- Switching from a race to VR settings or the controller tutorial no longer leaves an obsolete stereo request waiting for the 10-second OpenXR timeout. The pending request completes with a safe mono image, so the menu appears in the headset and closing it returns to the race promptly. VR panels also continue receiving frames when a paused game stops submitting GX display copies.
