...
struct RuntimeUserConfig {
    ...
    std::optional<float> vrDioramaDistance;
    std::optional<float> vrDioramaHeight;
    std::optional<float> vrDioramaUnitsPerMeter;
...
inline RuntimeUserConfig ParseConfigDocument(const toml::value& document) {
    ...
    config.vrStickCenterX = FindConfigFloat(document, "vr", "stick_center_x");
    config.vrStickCenterY = FindConfigFloat(document, "vr", "stick_center_y");
    config.vrDioramaDistance = FindConfigFloat(document, "vr", "diorama_distance");
    config.vrDioramaHeight = FindConfigFloat(document, "vr", "diorama_height");
    config.vrDioramaUnitsPerMeter = FindConfigFloat(document, "vr", "diorama_units_per_meter");
...
inline bool SetVrRenderScale(float value) {
    ...
    return WriteSetting("vr", "render_scale", std::to_string(value));
}

inline bool SetVrStickCalibration(float deadzone, float outer, float x, float y) {
    Mutable().vrStickDeadzone = std::clamp(deadzone, 0.0f, 0.4f);
    Mutable().vrStickOuter = std::clamp(outer, 0.6f, 1.0f);
    Mutable().vrStickCenterX = std::clamp(x, -0.3f, 0.3f);
    Mutable().vrStickCenterY = std::clamp(y, -0.3f, 0.3f);
    const bool a = WriteSetting("vr", "stick_deadzone", std::to_string(*Mutable().vrStickDeadzone));
    const bool b = WriteSetting("vr", "stick_outer", std::to_string(*Mutable().vrStickOuter));
    const bool c = WriteSetting("vr", "stick_center_x", std::to_string(*Mutable().vrStickCenterX));
    const bool d = WriteSetting("vr", "stick_center_y", std::to_string(*Mutable().vrStickCenterY));
    return a && b && c && d;
}
inline float VrDioramaDistance() { return std::clamp(Get().vrDioramaDistance.value_or(1600.0f), 200.0f, 5000.0f); }
inline float VrDioramaHeight() { return std::clamp(Get().vrDioramaHeight.value_or(1200.0f), 100.0f, 4000.0f); }
inline float VrDioramaUnitsPerMeter() { return std::clamp(Get().vrDioramaUnitsPerMeter.value_or(1000.0f), 100.0f, 3000.0f); }
inline bool SetVrDioramaDistance(float value) {
    Mutable().vrDioramaDistance = std::clamp(value, 200.0f, 5000.0f);
    return WriteSetting("vr", "diorama_distance", std::to_string(VrDioramaDistance()));
}
inline bool SetVrDioramaHeight(float value) {
    Mutable().vrDioramaHeight = std::clamp(value, 100.0f, 4000.0f);
    return WriteSetting("vr", "diorama_height", std::to_string(VrDioramaHeight()));
}
inline bool SetVrDioramaUnitsPerMeter(float value) {
    Mutable().vrDioramaUnitsPerMeter = std::clamp(value, 100.0f, 3000.0f);
    return WriteSetting("vr", "diorama_units_per_meter", std::to_string(VrDioramaUnitsPerMeter()));
}
...
inline float VrFirstPersonUnitsPerMeter(float fallback = 100.0f) {
