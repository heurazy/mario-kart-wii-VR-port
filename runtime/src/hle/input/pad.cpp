...
extern "C" uint32_t PAD__Read_HLE(uint32_t statusPtr)
{
    ...
    if (mkw::vr::ReadQuestPad(statuses[0], PADIsInputBlocked())) {
        // Quest owns player 1 while its input actions are active. Do not
        // advertise desktop rumble for this source; ports 2-4 stay untouched.
