...
void PublishVrSceneAnchor() {
    ...
    s_engaged = engaged;
    s_units_per_meter = anchor.units_per_meter;
    if (engaged && anchor.units_per_meter > 0.0f) {
...
void VI_HLE_PresentFrame(bool presentedXfb, bool paceToRetrace) {
    ...
    const uint64_t vrContentTag = vrPolicy.content_tag;
    aurora_end_frame_tagged(vrContentTag);
    const auto limiterStart = Clock::now();
    if (paceThisFrame) {
        PaceToRetraceBoundary(paceDeadline);
        std::lock_guard<std::mutex> lock(g_viMutex);
        s_lastPacedRetraceCount = g_vi.retraceCount;
    }
    const double limiterMs = std::chrono::duration<double, std::milli>(Clock::now() - limiterStart).count();
    ...
    // Separate game production from XR submissions: a headset running faster
    // than the guest does not by itself mean that simulation is running slow.
    static auto perfStart = Clock::now();
    static uint32_t perfFrames = 0;
    static double perfLimiterMs = 0.0;
    const auto now = Clock::now();
    if (vrPolicy.presentation == mkw::vr::VRPresentationMode::ImmersiveRace && presentedXfb) {
        ++perfFrames;
        perfLimiterMs += limiterMs;
        const double seconds = std::chrono::duration<double>(now - perfStart).count();
        if (seconds >= 5.0) {
            double targetHz;
            {
                std::lock_guard<std::mutex> lock(g_viMutex);
                targetHz = 1.0 / std::chrono::duration<double>(g_vi.retraceInterval).count();
            }
            RT_LOG(RT_TAG_RUNTIME) << "[mkw-vr] game timing: presented-fps=" << perfFrames / seconds
                << ", vi-target-hz=" << targetHz
                << ", extra-limiter-ms=" << perfLimiterMs / perfFrames << std::endl;
            perfStart = now;
            perfFrames = 0;
            perfLimiterMs = 0.0;
        }
